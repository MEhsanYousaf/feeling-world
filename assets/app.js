...
const THEMES = [
  {
    key: 'pink',
    name: 'Romantic Pink',
    tagline: 'Soft, warm & full of heart',
    particles: ['💗', '💕', '✨', '🌸']
  },
  {
    key: 'elegant',
    name: 'Elegant Dark',
    tagline: 'Midnight, refined & dreamy',
    particles: ['✦', '⭐', '🤍', '✧']
  },
  {
    key: 'cute',
    name: 'Cute & Playful',
    tagline: 'Bubbly, bright & fun',
    particles: ['🎈', '✨', '🎉', '💫']
  },
  {
    key: 'premium',
    name: 'Premium Gold',
    tagline: 'Luxe, minimal & timeless',
    particles: ['✨', '🥂', '⭐', '♛']
  }
];
...
function renderSwitcher(container, activeKey, onPick) {
  ...
  container.querySelectorAll('.swatch').forEach(el => {
    ...
    el.addEventListener('click', () => {
      ...
      onPick(
        el.getAttribute('data-theme-key')
      );
...
function spawnParticle(themeKey, layer) {
  ...
  const glyph =
    t.particles[
      Math.floor(
        Math.random() * t.particles.length
      )
    ];
